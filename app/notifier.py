...
import os, aiohttp, inspect
from datetime import datetime, timezone
from typing import Iterable, Optional

# ---- Discord Webhooks (ENV overridable) ----
...
async def post_signal_embed(*args, **kwargs):
    ...
    Old style:
        await post_signal_embed(cfg.discord_webhook,
                                exchange=..., symbol=..., interval=..., side=...,
                                price=..., vwap=..., rsi=..., score=..., triggers=[...])
    New style (preferred):
        await post_signal_embed(venue=..., symbol=..., interval=..., side=...,
                                price=..., vwap=..., rsi=..., score=..., triggers=[...], ts=..., signal_type="spot")
    """
    # ignore positional webhook (legacy first arg)
    if args:
        # the first positional arg used to be webhook_url; we ignore it
        if len(args) > 1:
            raise TypeError("post_signal_embed legacy call received unexpected extra positional args")
        # kwargs already contain the real fields
    # normalize keys
    ...
    vwap  = float(kwargs.pop("vwap"))
    rsi   = float(kwargs.pop("rsi"))
    score = float(kwargs.pop("score"))
    triggers = kwargs.pop("triggers", None)
    ts = kwargs.pop("ts", _now_iso())
    signal_type = kwargs.pop("signal_type", "spot")
    if kwargs:
        # surface any unexpected keys early
        raise TypeError(f"post_signal_embed got unexpected arguments: {list(kwargs.keys())}")
    ...
    embed = {
        ...
        "footer": {"text": f"QuickCap • {signal_type.upper()}"},
    }
...
async def post_performance_text(content: str):
    # plain text fallback (you’re also posting rich embeds via app/tools/report_to_discord.py)
