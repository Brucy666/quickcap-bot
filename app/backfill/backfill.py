...
async def backfill_symbol(
    ...
    alert_min_score: float, cooldown_sec: int, sqlite_path: str, supa: Supa | None,
) -> Dict[str, int]:
    ...
    for i in range(50, len(df)-1):
        ...
        if not triggers or last["score"] < alert_min_score:  continue
        ...
        sig_row = {
            "ts": _iso_utc(now), "signal_type":"spot", "venue":venue, "symbol":norm_symbol,
        ...
        nxt = df.iloc[i+1]
        exec_row = {
            "ts": _iso_utc(nxt["ts"].timestamp()), "venue":"PAPER", "symbol":norm_symbol,
    ...
    try:
        await post_backfill_summary(venue, norm_symbol, interval, sig_ct, exe_ct, len(out_rows))
    except Exception as e:
...
async def main():
    ...
    # Optional: auto performance report to Discord (if env/webhook exists)
    try:
