...
from app.storage.supabase import Supa  # <-- uses your existing client
...
async def backfill_symbol(venue: str, symbol: str, interval: str, lookback: int,
    ...
    # Optional Supabase mirror (enabled via env)
    cfg = load_settings()
    supa = None
    ...
    for i in range(50, len(df)-1):
        ...
        signal_id = store.insert_signal(signal_row); sig_ct += 1
        if supa:
            # fire & forget
