# app/backtest/metrics.py
from __future__ import annotations
from datetime import datetime
from typing import Iterable, List, Dict
import pandas as pd
from app.storage.sqlite_store import SQLiteStore

HORIZONS_MIN: tuple[int, ...] = (15, 30, 60)

def _dir(side: str) -> int: return 1 if str(side).upper() == "LONG" else -1

def _ts_ms(x) -> int:
    if isinstance(x, pd.Timestamp):
        return int((x.tz_localize(None) if x.tzinfo else x).timestamp() * 1000)
    if isinstance(x, str):
        return int(datetime.fromisoformat(x.replace("Z","+00:00")).timestamp() * 1000)
    return int(float(x) * 1000)

def _nearest_index(df: pd.DataFrame, ts_ms: int) -> int:
    arr = (df["ts"].astype("int64") // 10**6).to_numpy()
    lo, hi = 0, len(arr)-1
    while lo <= hi:
        mid = (lo+hi)//2
        if arr[mid] < ts_ms: lo = mid+1
        else: hi = mid-1
    return lo if lo < len(arr) else -1

def _calc_window(df: pd.DataFrame, i_entry: int, bars: int, side: str, entry: float):
    d = _dir(side); i1 = min(len(df)-1, i_entry + max(1, bars))
    seg = df.iloc[i_entry:i1+1]
    exit_price = float(seg.iloc[-1]["close"])
    ret = (exit_price / entry - 1.0) * d
    hi = float(seg["high"].max()); lo = float(seg["low"].min())
    if d > 0:
        mfe = max(0.0, hi/entry - 1.0)
        mae = max(0.0, entry/lo - 1.0)
    else:
        mfe = max(0.0, entry/lo - 1.0)
        mae = max(0.0, hi/entry - 1.0)
    return ret, mfe, mae, exit_price

def compute_outcomes_from_df(
    df: pd.DataFrame, venue: str, symbol: str, interval: str,
    store: SQLiteStore, horizons: Iterable[int] = HORIZONS_MIN
) -> List[Dict]:
    if df is None or df.empty: return []
    need = {"ts","open","high","low","close"}
    if not need.issubset(df.columns): raise ValueError(f"DF missing {need - set(df.columns)}")
    df = df.sort_values("ts", ignore_index=True).reset_index(drop=True)

    with store._conn() as con:
        sigs = con.execute(
            "SELECT id, ts, side FROM signals WHERE venue=? AND symbol=? AND interval=? ORDER BY ts ASC",
            (venue, symbol, interval)
        ).fetchall()
    if not sigs: return []

    try: step = int(interval.rstrip("m"))
    except: step = 1

    out: List[Dict] = []
    for sid, ts_iso, side in sigs:
        i0 = _nearest_index(df, _ts_ms(ts_iso))
        if i0 < 0: continue
        i_entry = min(i0+1, len(df)-1)
        entry = float(df.iloc[i_entry]["open"])
        for h in horizons:
            bars = max(1, int(h // step))
            ret, mfe, mae, exit_price = _calc_window(df, i_entry, bars, side, entry)
            out.append({
                "signal_id": int(sid), "horizon_m": int(h),
                "entry_price": float(entry), "exit_price": float(exit_price),
                "ret": float(ret), "max_fav": float(mfe), "max_adv": float(mae),
            })
    if out: store.upsert_outcomes(out)
    return out

# backward-compat wrapper (if some call sites still import the old name)
async def compute_outcomes_sqlite_rows(venue, symbol, interval, lookback, store, df: pd.DataFrame|None=None):
    return compute_outcomes_from_df(df, venue, symbol, interval, store) if df is not None else []
